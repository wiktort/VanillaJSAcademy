...
const checkMonster = (monster) => {
    ...
    const showResult = (result) => {
        ...
        : popupGameOver.style.visibility = "visible";
    };
    // check if all of the monsters are shown and return true or false
    const monsters = [...monster.closest(".row").childNodes];
    const checkAll = () => {
        return monsters.length === monsters.reduce((acc,child) =>{
            return acc += child.classList.contains("active");
        },0);
    };
    // check the last last monster. If it was a sock, call showResult() with boolean from checkAll()
    monster.id.includes("sock") ? showResult(checkAll()) : 0;
};

const game = (e)=> {
    //close popup
    if(e.target.classList.contains("closepopup")){
        popupGameOver.style.visibility = "hidden";
    ...
    if(e.target.classList.contains("grid")){
        //hide the current button and add class "active"
